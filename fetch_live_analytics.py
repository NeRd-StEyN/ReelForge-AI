...
from pipeline.insta_handler import get_insta_client, get_graph_api_performance_data
...
reels = []
# Priority 1: Check official Instagram Graph API (for Professional / Creator accounts)
graph_reels = get_graph_api_performance_data(limit=25)
if graph_reels:
    print("[Step 1] Graph API authenticated! Using official Meta Graph API.")
    reels = graph_reels
else:
    print("\n[Step 1] Logging into Instagram via Instagrapi session...")
    cl = get_insta_client()
    if not cl:
        print("ERROR: Could not get Instagram client. Check insta_session.json or INSTAGRAM_GRAPH_TOKEN in .env")
        sys.exit(1)
    ...
    try:
        ...
        if "Please wait a few minutes" in str(e):
            ...
            print("[Instagram Cooldown] Solution: Wait 5 to 10 minutes for the cooldown to reset, or configure INSTAGRAM_GRAPH_TOKEN.")
        else:
...
for i, r in enumerate(reels, 1):
    ...
    print("  #" + str(i) + pinned_str + " | " + str(r.get("_taken_at", r.get("post_date", ""))))
