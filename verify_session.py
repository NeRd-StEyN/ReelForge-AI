...
if not os.path.exists(session_file):
    print(f"\n[FAIL]: Session file '{session_file}' NOT FOUND")
    sys.exit(1)
...
try:
    ...
    print(f"\n[FAIL]: Session file is CORRUPTED (invalid JSON)")
    print(f"   Error: {e}")
    sys.exit(1)

print(f"\n[OK] Session file is valid JSON")
...
print(f"   {'[OK]' if has_cookies else '[FAIL]'} Cookies: {len(cookies)} items" if has_cookies else "   [FAIL] Cookies: MISSING")
...
for key in required_keys:
    status = "[OK]" if key in data else "[FAIL]"
    if key not in data:
        structure_ok = False
    print(f"   {status} {key}")
...
if session_age_hours < 24:
    print(f"   [OK] Fresh (generated {session_age_hours:.1f} hours ago)")
    age_ok = True
elif session_age_hours < 7*24:
    print(f"   [WARN] Recent ({session_age_hours:.1f} hours ago)")
    age_ok = True
elif session_age_hours < 30*24:
    print(f"   [WARN] Getting old ({session_age_hours/24:.0f} days ago - regenerate soon)")
    age_ok = True
else:
    print(f"   [FAIL] EXPIRED ({session_age_hours/24:.0f} days ago - regenerate NOW)")
    age_ok = False

# Final verdict
print("\n" + "=" * 70)
if all_present and has_cookies and device_id and structure_ok:
    print("[OK] VERDICT: Session is VALID and ready for GitHub Actions")
    ...
    print("   4. Check logs for: [OK] Session validation passed")
    ...
    print("[FAIL] VERDICT: Session has issues")
    print(f"\nIssues found:")
    if not device_id:
        print("   [FAIL] Device ID missing")
    if not phone_id:
        print("   [FAIL] Phone ID missing")
    if not has_cookies:
        print("   [FAIL] Cookies missing")
    if not structure_ok:
        print("   [FAIL] Session structure incomplete")
    if not age_ok:
        print("   [FAIL] Session expired or too old")
